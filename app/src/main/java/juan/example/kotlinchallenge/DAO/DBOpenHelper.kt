package juan.example.kotlinchallenge.DAO
...
class DBOpenHelper(context: Context, factory: SQLiteDatabase.CursorFactory?) :
    ...
    companion object{
        private const val DATABASE_NAME = "Pelicula.db"
        ...
        const val COLUMN_IDIOMA_ORIGINAL = "idioma_original"
        const val COLUMN_DURACION = "duracion"
        const val COLUMN_VOTACION_PROMEDIO = "votacion_promedio"
        const val COLUMN_ESTADO = "estado"
        const val COLUMN_SINOPSIS = "sinopsis"
        const val COLUMN_GENEROS = "generos"
        const val COLUMN_COMPAÑIAS_PRODUCCION = "compañias_produccion"
        const val COLUMN_PAISES_PRODUCCION = "paises_produccion"
    }
    ...
    fun obtenerPelicula(id: Int): Pelicula?
    {
        ...
        if (cursor.moveToFirst())
        {
            ...
            val compañiasProduccion = cursor.getString(cursor.getColumnIndex(
                COLUMN_COMPAÑIAS_PRODUCCION
            ))
